...
function compile_kinematicsData(dataStruct, fileNames, pathName)
    ...
    for i = 1:lengthDataStruct
        ...
        ahm = anteriorHyoidMovement(doubleCell);
        shm = superiorHyoidMovement(doubleCell, phaseFramesCell);
        hyExMand = hyoidExcursionToMandible(doubleCell);
        hyExC4 = hyoidExcursionToC4(doubleCell);
        hyExVert = hyoidExcursionToVertebrae(doubleCell);
        alm = antLargyngealMovement(doubleCell);
...
function ahm = anteriorHyoidMovement(doubleCell, scalar)
...
function shm = superiorHyoidMovement(doubleCell, phaseFramesCell)
    ...
    for i = 1:doubleCellSize(1)
        ...
        scalar = pdist(c2c4_points,'euclidean');
        ...
        allVertMovements(i) = current_orthogonal_dist / scalar;
    ...
    shm = max(allVertMovements) - min(allVertMovements(allVertMovements>0));
end
