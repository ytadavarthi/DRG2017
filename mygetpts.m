function [x,y] = mygetpts(varargin)
...
if (nargin < 1)
    ...
    if (~ishghandle(varargin{1}))
        error(message('images:mygetpts:expectedHandle'));
    end
    ...
    otherwise
        error(message('images:mygetpts:expectedFigureOrAxesHandle'));
...
set(GETPTS_FIG, 'WindowButtonDownFcn', 'mygetpts(''FirstButtonDown'');', ...
        'KeyPressFcn', 'mygetpts(''KeyPress'');', ...
        'Pointer', 'custom', ...
...
case 'trap'
    ...
    error(message('images:mygetpts:interruptedMouseSelection'));
...
case {char(13), char(3), char('p')}   % enter and return keys
...
if (~strcmp(get(GETPTS_FIG, 'SelectionType'), 'normal'))
    ...
    set(GETPTS_FIG, 'WindowButtonDownFcn', 'mygetpts(''NextButtonDown'');');
end
